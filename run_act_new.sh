. dcuevas_hf_token.sh
rm -rf /home/sinfonia/.cache/huggingface/lerobot/local/eval_act_cloud_test
...
lerobot-record \
  ...
  --dataset.repo_id=local/eval_act_cloud_test \
  --dataset.num_episodes=1 \
  --dataset.single_task="Put the cube on the tray" \
  --dataset.push_to_hub=false \
  --policy.type=act \
  --policy.pretrained_path=bendca61/act-so101-mujoco-cube_on_tray-v1 \
  --policy.device=cuda \
  --display_data=true