. dcuevas_hf_token.sh
rm -rf /home/sinfonia/.cache/huggingface/lerobot/local/eval_smolvla_cloud_test
...
lerobot-record \
  ...
  --dataset.repo_id=local/eval_smolvla_cloud_test \
  --dataset.num_episodes=1 \
  --dataset.single_task="Put the cube on the tray" \
  --dataset.push_to_hub=false \
  --policy.type=smolvla \
  --policy.pretrained_path=bendca61/smolvla-mujoco-so101-cube_on_tray \
  --display_data=true